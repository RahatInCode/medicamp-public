    ...
    <nav className="bg-white dark:bg-gray-900 shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-2 font-bold text-xl">
          ...
          <span className="text-black dark:text-white">MediCamp</span>
        </Link>
        ...
        <div className="hidden sm:flex items-center gap-4">
          ...
          {!user ? (
            ...
            <Menu as="div" className="relative">
              ...
              >
                <Menu.Items className="absolute right-0 mt-2 w-64 origin-top-right bg-white dark:bg-gray-800 divide-y divide-gray-100 rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                      ...
                      {({ active }) => (
                        <button
                          ...
                          className={`block w-full text-left px-4 py-2 text-sm rounded-md ${
                            active ? "bg-red-100 dark:bg-red-700" : "text-red-600 dark:text-red-400"
                          }`}
                        >
                          Logout
                        </button>
      ...
      {menuOpen && (
        <div className="sm:hidden px-4 pb-4 space-y-2">
          <Link to="/" className="block text-gray-700 dark:text-gray-300 font-medium">
            Home
          </Link>
          <Link to="/camps" className="block text-gray-700 dark:text-gray-300 font-medium">
            Available Camps
          </Link>
          
          {user ? (
            <>
              <Link to={dashboardPath} className="block text-gray-700 dark:text-gray-300 font-medium">
                Dashboard
              </Link>
              <button
                onClick={handleLogout}
                className="block text-red-600 dark:text-red-400 font-medium"
              >
                Logout
              </button>
            </>
